...
def nms(dets, thresh, cfg):
    """Dispatch to either CPU or GPU NMS implementations."""

    if dets.shape[0] == 0:
        return []
    if cfg.MAIN_DEFAULT_USE_GPU_NMS:
        return gpu_nms(dets, thresh, device_id=cfg.MAIN_DEFAULT_GPU_ID)
    else:
...
def apply_nms_mask_single(box, mask, thresh, cfg):
    if box == []:
        return box, mask
    keep = nms(box, thresh, cfg)
    if len(keep) == 0:
        return box, mask
